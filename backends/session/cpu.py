    ...
    def __connect_cpu(self):
        ...
        while retry > 0:
            try:
                ...
                retry = retry - 1
        sessionlog.write(
            "ERROR: 'Cpu.__connect_cpu()' - 3 attempts to connect to system bus failed.")
        return None
    ...
    def __connect_fan(self):
        ...
        while retry > 0:
            try:
                bus = dbus.SystemBus()
                proxy = bus.get_object(
                    SYSTEM_INTERFACE_NAME, SYSTEM_OBJECT_PATH_FAN)
                return dbus.Interface(proxy, SYSTEM_INTERFACE_NAME)
            except:
                retry = retry - 1
        sessionlog.write(
            "ERROR: 'Cpu.__connect_fan()' - 3 attempts to connect to system bus failed.")
        return None
