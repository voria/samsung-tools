...
class Wireless(dbus.service.Object):
	...
	def __connect(self):
		...
		while retry > 0:
			try:
				bus = dbus.SystemBus()
				proxy = bus.get_object(SYSTEM_INTERFACE_NAME, SYSTEM_OBJECT_PATH_WIRELESS)
				return dbus.Interface(proxy, SYSTEM_INTERFACE_NAME)
			except:
				retry = retry - 1
		sessionlog.write("ERROR: 'Wireless.__connect()' - 3 attempts to connect to system bus failed.")
		return None
