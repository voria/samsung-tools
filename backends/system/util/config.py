...
CPUFAN_INITIAL_STATUS_ACCEPTED_VALUES = [
    'normal', 'silent', 'overclock', 'last']
    ...
    def __write(self, option):
        ...
        try:
            ...
            try:
                ...
                systemlog.write(
                    "ERROR: 'SystemConfig.__write()' - cannot write the config file.")
                return False
        try:
            newfile = open(self.configfile + ".new", "w")
        except:
            systemlog.write(
                "ERROR: 'SystemConfig.__write()' - cannot write the new config file.")
            oldfile.close()
            return False
        for line in oldfile:
            if line[0:1] == "#" or line == "\n":
